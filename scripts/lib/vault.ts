// uebernommen aus 3d-codeblocks/scripts/lib/vault.ts, 2026-08-16 — Fixture→Vault; Abweichung: Generator heisst make-audio.mjs statt make-models.mjs (deklariert)
/**
...
export function buildVault(spec: VaultSpec): string[] {
  ...
  const modelle = execFileSync("node", [join(fixtureDir, "make-audio.mjs"), vaultDir], {
    encoding: "utf-8",
  });
  log.push("Audio erzeugt:" + modelle.replace(/\n$/, "").replace(/\n/g, "\n   "));
